...
function storeSiteData(opts, city, err, data) {
	...
	const tr = new TempRecord(siteName + '/' + city, reqTime, temps)
	if (!opts.storeTemps) {
...
function poll(sites, storingOpts, cities) {
	for (let city of cities) {
		for (let site of sites) {
			site.opts.path = `/pogoda/${city}/details?via=ms`
			const storeCurrentSiteData = storeSiteData.bind(null, storingOpts, city);
			getTempFrom(site, storeCurrentSiteData);
		}
	}
}

const cities = fs.readFileSync('./crawler/cities.txt', 'utf8').toLocaleLowerCase().split('\r\n')
console.log(cities)
